...
$none_entity = "None";
$type = ($databanks->type_opportunity_id) ? \App\Models\TypeOpportunity::find($databanks->type_opportunity_id) : null;
$prescripteur = ($databanks->prescripteur_id) ? \App\Models\Prescripteur::find($databanks->prescripteur_id) : null;
$pays_partner = DB::table('pays_partners')
->select('pays.fr as libelle')
->join('pays', 'pays.id', 'pays_partners.pays_id')
->where('pays_partners.id', $databanks->pays_partner_id)
->first();
$started_at = ($databanks->started_at) ?: $none_entity;
$deadline = ($databanks->deadline) ?: $none_entity;
$image_url = ($databanks->image_url) ?: asset('images/a-la-une-default.png');
$source = ($databanks->source) ?: null;

$attachments = \App\Models\Attachment::where('databank_id', $databanks->id)->get();
...
<div class="col-sm-12">
    ...
    <p>{{ $databanks->description }}</p>
</div>

<div class="col-sm-12">
    {!! Form::label('criteres', 'Criteres :') !!}
    <p>{!! ($databanks->criteres) ?: '<small style="color:red">Aucun critere signale !</small>' !!}</p>
</div>
